...
for _ in range(test):
    ...
    for key in range(order_num):
        ...
        if order[0] == 'I':
            ...
            visit[key] = 1 #True이면 어떤 힙에서도 아직 삭제되지 않은 상태
            ...
            if order[1] == '-1': #삭제연산시, key값을 기준으로 해당 노드가 다른힙에서 삭제된 노드인가를 먼저 판단한다.
                ...
                while min_heap and not visit[min_heap[0][1]]: # visit이 False일때 -> 해당노드가 삭제된상태
                    heapq.heappop(min_heap) # 버림 (상대힙에서 이미 삭제된노드이므로)
                if min_heap:
                    visit[min_heap[0][1]] = 0 # visit이 Ture 였으므로 False로 바꾸고 삭제함
